...
(function () {
  ...
  const labels = () => isGerman() ? {
    ...
    hint:'Die vollständigsten Metadaten wurden vorausgewählt. Bitte vor dem Speichern prüfen.', titleField:'Titel', authorField:'Autor', originalTitle:'Originaltitel', genre:'Genre', year:'Jahr', language:'Sprache', originalLanguage:'Originalsprache', series:'Reihe', summary:'Kurzbeschreibung', mainIdea:'Kernidee'
  } : {
    ...
    hint:'The most complete metadata was preselected. Review it before saving.', titleField:'Title', authorField:'Author', originalTitle:'Original title', genre:'Genre', year:'Year', language:'Language', originalLanguage:'Original language', series:'Series', summary:'Summary', mainIdea:'Main idea'
  };
  ...
  function showSearchStatus() {
    document.getElementById('android-duplicate-search-status')?.remove();
    const L=labels();
    const busy=document.createElement('div');
    busy.id='android-duplicate-search-status';
  ...
  function hideSearchStatus(){ document.getElementById('android-duplicate-search-status')?.remove(); }
  ...
  function mergeCandidate(entries){
    ...
    keys.forEach(key=>{
      if(Array.isArray(merged[key])){const out=[],seen=new Set();books.forEach(b=>(Array.isArray(b[key])?b[key]:[]).forEach(v=>{const k=JSON.stringify(v);if(!seen.has(k)){seen.add(k);out.push(v);}}));merged[key]=out;}
      else if(empty(merged[key])){for(const b of books){if(!empty(b[key])){merged[key]=b[key];break;}}}
    });
    delete merged.confidence;return merged;
  }
  function field(label,id,value,multi){
    const control=multi?`<textarea id="${id}" rows="4" style="box-sizing:border-box;width:100%;border:1px solid var(--rule);background:transparent;padding:10px;font-family:var(--serif);font-size:15px;color:var(--ink)">${esc(value||'')}</textarea>`:`<input id="${id}" value="${esc(value||'')}" style="box-sizing:border-box;width:100%;border:1px solid var(--rule);background:transparent;padding:10px;font-family:var(--serif);font-size:15px;color:var(--ink)"/>`;
  ...
  function editSheet(entry,onDone){
    ...
    sheet.innerHTML=`<div class="display" style="font-size:24px;margin-bottom:12px">${esc(L.edit)}</div>${field(L.titleField,'e-title',b.title)}${field(L.authorField,'e-author',b.author)}${field(L.genre,'e-genre',genre)}${field(L.year,'e-year',b.year_published)}${field(L.language,'e-language',b.language)}${field(L.summary,'e-summary',b.summary,true)}${field(L.mainIdea,'e-main',b.main_idea,true)}<div id="e-error" style="display:none;color:var(--oxblood);padding:10px 0"></div><button id="e-save" style="width:100%;text-align:left;border-top:1px solid var(--ink);padding:15px 2px;color:var(--oxblood)">${esc(L.save)}</button><button id="e-cancel" style="width:100%;text-align:left;border-top:1px solid var(--rule);padding:15px 2px">${esc(L.cancel)}</button>`;
  ...
  function closeOverview(overlay){
    overlay.remove();
    if(catalogChanged){catalogChanged=false;native.reloadLibrary();}
  }

  function refreshOverview(overlay,sheet){
    const L=labels();
    let result;try{result=JSON.parse(native.findDuplicateBooks());}catch(e){result={ok:false,error:String(e)}}
    const groups=result&&Array.isArray(result.groups)?result.groups:[];
    if(!result.ok||!groups.length){sheet.innerHTML=`<div class="display" style="font-size:24px;margin-bottom:18px">${esc(L.title)}</div><div style="font-family:var(--serif);font-size:17px;margin-bottom:22px">${esc(result&&result.ok?L.none:((result&&result.error)||L.error))}</div><button id="d-close" style="width:100%;border-top:1px solid var(--rule);padding:15px 2px;text-align:left">${esc(L.close)}</button>`;sheet.querySelector('#d-close').onclick=()=>closeOverview(overlay);return;}
    const groupsHtml=groups.map((g,gi)=>`<section style="margin-bottom:28px"><div style="font-family:var(--mono);font-size:9px;color:var(--ink-3)">${esc(L.similarity)} ${Math.round((Number(g.confidence)||0)*100)}% · ${esc(g.reason||'')}</div><div class="display" style="font-size:20px;margin:5px 0 8px">${esc(g.title||'')}</div>${(g.entries||[]).map((e,ei)=>`<div style="display:grid;grid-template-columns:28px 1fr auto;gap:8px;align-items:start;padding:11px 0;border-top:1px solid var(--rule)"><input class="dup-check" type="checkbox" data-group="${gi}" data-index="${Number(e.index)}" ${ei<2?'checked':''} style="width:18px;height:18px;margin-top:3px"/><div><div style="font-family:var(--serif);font-size:16px">${esc(e.title)}</div><div style="font-size:12px;color:var(--ink-2)">${esc(e.author)}</div></div><button class="dup-edit" data-group="${gi}" data-entry="${ei}" style="font-family:var(--mono);font-size:9px;padding:7px">${esc(L.edit)}</button></div>`).join('')}<button class="dup-merge" data-group="${gi}" style="width:100%;text-align:left;border-top:1px solid var(--ink);padding:12px 2px;color:var(--oxblood)">${esc(L.merge)}</button></section>`).join('');
    sheet.innerHTML=`<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:18px"><div class="display" style="font-size:24px">${esc(L.title)}</div><button id="d-close" style="padding:8px">${esc(L.close)}</button></div>${groupsHtml}<div id="d-confirm" style="display:none;color:var(--oxblood);padding:12px 0">${esc(L.confirm)}</div><button id="d-delete" style="width:100%;text-align:left;border-top:1px solid var(--ink);padding:15px 2px;color:var(--oxblood)">${esc(L.del)}</button>`;
    sheet.scrollTop=0;
    sheet.querySelector('#d-close').onclick=()=>closeOverview(overlay);
    const checks=()=>Array.from(sheet.querySelectorAll('.dup-check'));
    const refresh=()=>window.setTimeout(()=>refreshOverview(overlay,sheet),0);
    sheet.querySelectorAll('.dup-edit').forEach(btn=>btn.onclick=()=>{const e=groups[Number(btn.dataset.group)].entries[Number(btn.dataset.entry)];editSheet(e,refresh);});
    sheet.querySelectorAll('.dup-merge').forEach(btn=>btn.onclick=()=>{const gi=Number(btn.dataset.group),chosen=checks().filter(c=>c.checked&&Number(c.dataset.group)===gi).map(c=>groups[gi].entries.find(e=>Number(e.index)===Number(c.dataset.index))).filter(Boolean);if(chosen.length<2){btn.textContent=L.merge+' (2+)';return;}mergeSheet(chosen,refresh);});
    let armed=false;const confirm=sheet.querySelector('#d-confirm');sheet.querySelector('#d-delete').onclick=()=>{const indices=checks().filter(c=>c.checked).map(c=>Number(c.dataset.index));if(!indices.length)return;if(!armed){armed=true;confirm.style.display='block';return;}let r;try{r=JSON.parse(native.deleteBookEntries(JSON.stringify(indices)));}catch(e){r={ok:false,error:String(e)}}if(!r.ok){confirm.textContent=r.error||L.error;armed=false;return;}catalogChanged=true;refresh();};
  }

  function openDuplicates(){
    hideSearchStatus();
    const {overlay,sheet}=sheetBase('android-duplicates-overlay',360);
    overlay.addEventListener('click',e=>{if(e.target===overlay)closeOverview(overlay);});
    refreshOverview(overlay,sheet);
  }
  ...
  function injectSettingsAction(){
    ...
    const anchor=overlay.querySelector('#android-export-json');if(!anchor)return;
    const button=document.createElement('button');button.id='android-find-duplicates';button.type='button';button.textContent=labels().action;
    ...
    button.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();overlay.remove();showSearchStatus();window.setTimeout(openDuplicates,80);});
