...
(function () {
  ...
  function ignoredPairs() {
    try { const v=JSON.parse(localStorage.getItem(IGNORE_KEY)||'[]'); return new Set(Array.isArray(v)?v:[]); }
  ...
  function renderDuplicates(overlay,sheet){
    ...
    if(!result||!result.ok||!groups.length){sheet.innerHTML=`<div class="display" style="font-size:24px;margin-bottom:18px">${esc(L.title)}</div><div style="font-family:var(--serif);font-size:17px;margin-bottom:22px">${esc(result&&result.ok?L.none:((result&&result.error)||L.error))}</div><button id="d-close" style="width:100%;border-top:1px solid var(--rule);padding:15px 2px;text-align:left">${esc(L.close)}</button>`;sheet.querySelector('#d-close').onclick=close;return;}
    const html=groups.map((g,gi)=>`<section class="dup-group" data-group="${gi}" style="margin-bottom:28px"><div style="font-family:var(--mono);font-size:9px;color:var(--ink-3)">${esc(L.similarity)} ${Math.round((Number(g.confidence)||0)*100)}% · ${esc(g.reason||'')}</div><div class="display" style="font-size:20px;margin:5px 0 8px">${esc(g.title||'')}</div>${(g.entries||[]).map((e,ei)=>`<div style="display:grid;grid-template-columns:28px 1fr auto;gap:8px;align-items:start;padding:11px 0;border-top:1px solid var(--rule)"><input class="dup-check" type="checkbox" data-group="${gi}" data-index="${Number(e.index)}" ${ei<2?'checked':''} style="width:18px;height:18px;margin-top:3px"/><div><div style="font-family:var(--serif);font-size:16px">${esc(e.title)}</div><div style="font-size:12px;color:var(--ink-2)">${esc(e.author)}</div></div><button class="dup-edit" data-group="${gi}" data-entry="${ei}" style="font-family:var(--mono);font-size:9px;padding:7px">${esc(L.edit)}</button></div>`).join('')}<button class="dup-merge" data-group="${gi}" style="width:100%;text-align:left;border-top:1px solid var(--ink);padding:12px 2px;color:var(--oxblood)">${esc(L.merge)}</button><button class="dup-not" data-group="${gi}" style="width:100%;text-align:left;border-top:1px solid var(--rule);padding:12px 2px">${esc(L.notDuplicate)}</button></section>`).join('');
    sheet.innerHTML=`<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:18px"><div class="display" style="font-size:24px">${esc(L.title)}</div><button id="d-close" style="padding:8px">${esc(L.close)}</button></div>${html}<div id="d-confirm" style="display:none;color:var(--oxblood);padding:12px 0">${esc(L.confirm)}</div><button id="d-delete" style="width:100%;text-align:left;border-top:1px solid var(--ink);padding:15px 2px;color:var(--oxblood)">${esc(L.del)}</button>`;
    sheet.querySelector('#d-close').onclick=close;
    const checks=()=>Array.from(sheet.querySelectorAll('.dup-check'));
    sheet.querySelectorAll('.dup-edit').forEach(btn=>btn.onclick=()=>{const e=groups[Number(btn.dataset.group)].entries[Number(btn.dataset.entry)];editSheet(e,()=>renderDuplicates(overlay,sheet));});
    sheet.querySelectorAll('.dup-merge').forEach(btn=>btn.onclick=()=>{const gi=Number(btn.dataset.group),chosen=checks().filter(c=>c.checked&&Number(c.dataset.group)===gi).map(c=>groups[gi].entries.find(e=>Number(e.index)===Number(c.dataset.index))).filter(Boolean);if(chosen.length<2){btn.textContent=L.merge+' (2+)';return;}mergeSheet(chosen,()=>renderDuplicates(overlay,sheet));});
    sheet.querySelectorAll('.dup-not').forEach(btn=>btn.onclick=()=>{ignoreGroup(groups[Number(btn.dataset.group)]);renderDuplicates(overlay,sheet);});
