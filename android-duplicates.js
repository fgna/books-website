...
(function () {
  ...
  const labels = () => isGerman() ? {
    action:'Duplikate suchen', searching:'Duplikate werden gesucht…', ignoring:'Duplikat wird ausgeblendet…', merging:'Einträge werden zusammengeführt…', title:'Mögliche Duplikate', none:'Keine möglichen Duplikate gefunden.', close:'Schließen',
    ...
    action:'Find duplicates', searching:'Searching for duplicates…', ignoring:'Hiding duplicate suggestion…', merging:'Merging entries…', title:'Possible duplicates', none:'No possible duplicates found.', close:'Close',
  ...
  function mergeSheet(entries,onDone){
    ...
    sheet.querySelector('#m-save').onclick=()=>{const v=id=>sheet.querySelector(id).value.trim();b.title=v('#m-title');b.author=v('#m-author');b.original_title=v('#m-original')||null;b.genre=v('#m-genre').split(',').map(x=>x.trim()).filter(Boolean);b.language=v('#m-language');b.original_language=v('#m-original-language')||null;b.series=v('#m-series')||null;b.summary=v('#m-summary');b.main_idea=v('#m-main')||null;const y=v('#m-year');b.year_published=y&&Number.isFinite(Number(y))?Number(y):null;const busy=busyScreen(L.merging);window.setTimeout(()=>{let r;try{r=JSON.parse(native.mergeBookEntries(JSON.stringify(entries.map(e=>Number(e.index))),JSON.stringify(b)));}catch(e){r={ok:false,error:String(e)}}if(!r.ok){busy.remove();const box=sheet.querySelector('#m-error');box.style.display='block';box.textContent=r.error||L.error;return;}catalogChanged=true;overlay.remove();onDone();busy.remove();},40);};
  }
